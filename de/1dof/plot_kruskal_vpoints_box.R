...
plot_kw_boxplot = function(rdafolder,vardata,alpha,plottype){
  ...
  data<-bind_rows(dtde1,dtde2,dtde3,dtde4)
  ...
  # EXIBINDO OS DADOS
  datastat=data.frame(Repet = data.nrep, Medias = data.mds, Mediana = data.mda, Variancias = data.var, EP = data.se, row.names = c("DE1","DE2","DE3","DE4"))
  ...
  kwframe<-data.frame(x=c("DE1","DE2","DE3","DE4"),
                      y=datastat$Mediana,
                      l=toupper(letters));
