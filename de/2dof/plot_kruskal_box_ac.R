...
if(plottype!="boxplot"){
  ...
  dirplot<-paste(rdafolder,"plots","kw_boxplot",plottype,'ac',sep="/")
  ...
  imgfilename=paste(vardata,plottype,"mkw_edac_a",alpha,rdafolder,".png",sep="_")
  ...
  #########GGPLOT BOX PLOT ##############
  kwframe<-data.frame(x=print(kt.cldList$Group),
                      y=datastat$Mediana,
                      l=toupper(letters));
  cls=c("A"="red","B"="green","AB"="green","C"="blue","BC"="purple","AC"="orange","ABC"="black")
  ...
  p<-ggplot()+
    ...
    geom_boxplot(aes(algo,dtdec1[[vardata]]),dtdec1,color=cl2,stat = "boxplot")+
    geom_boxplot(aes(algo,dtdea2[[vardata]]),dtdea2,color=cl3,stat = "boxplot")+
    geom_boxplot(aes(algo,dtdec2[[vardata]]),dtdec2,color=cl4,stat = "boxplot")+
